...
const MUNICIPALITY_MAP = {
  ...
  // 関東
  '水戸': '茨城県', 'つくば': '茨城県', '坂東': '茨城県', '日立': '茨城県', '土浦': '茨城県', '神栖': '茨城県', '足利': '栃木県', '宇都宮': '栃木県', '小山': '栃木県', '鹿沼': '栃木県',
...
const FOREIGN_KEYWORDS = [
  ...
  'ベトナム', '中国', '韓国', '台湾', '香港', '北朝鮮', '朝鮮籍', 'モンゴル',
  'フィリピン', 'タイ人', 'タイ国籍', 'タイ籍', 'インドネシア', 'カンボジア', 'ミャンマー', 'ラオス', 'マレーシア', 'シンガポール', 'ブルネイ', '東ティモール',
  'ネパール', 'インド', 'スリランカ', 'パキスタン', 'バングラデシュ', 'ブータン', 'モルディブ', 'アフガニスタン',
  'ウズベキスタン', 'カザフスタン', 'キルギス', 'タジキスタン', 'トルクメニスタン',

  // 中東・西アジア
  'トルコ', 'クルド', 'イラン', 'イラク', 'シリア', 'レバノン', 'ヨルダン', 'イスラエル', 'パレスチナ',
  'サウジアラビア', 'アラブ首長国連邦', 'UAE', 'カタール', 'クウェート', 'オマーン', 'イエメン', 'バーレーン',
  'ジョージア', 'アルメニア', 'アゼルバイジャン',

  // 米州（北米・中南米・カリブ）
  'アメリカ', '米国', '米国籍', 'カナダ',
  'ブラジル', 'ペルー', 'コロンビア', 'アルゼンチン', 'チリ', 'ボリビア', 'パラグアイ', 'ウルグアイ', 'エクアドル', 'ベネズエラ', 'ガイアナ', 'スリナム',
  'メキシコ', 'グアテマラ', 'エルサルバドル', 'ホンジュラス', 'ニカラグア', 'コスタリカ', 'パナマ',
  'ドミニカ', 'ドミニカ共和国', 'ジャマイカ', 'キューバ', 'ハイチ', 'バハマ', 'トリニダード・トバゴ',

  // ヨーロッパ
  'イギリス', '英国', 'フランス', 'ドイツ', 'イタリア', 'スペイン', 'ポルトガル', 'オランダ', 'ベルギー', 'スイス', 'オーストリア', 'アイルランド', 'ギリシャ',
  'ロシア', 'ウクライナ', 'ベラルーシ', 'ポーランド', 'チェコ', 'スロバキア', 'ハンガリー', 'ルーマニア', 'ブルガリア',
  'セルビア', 'クロアチア', 'ボスニア', 'アルバニア', '北マケドニア', 'コソボ', 'モルドバ',
  'スウェーデン', 'ノルウェー', 'フィンランド', 'デンマーク', 'アイスランド',
  'エストニア', 'ラトビア', 'リトアニア',

  // アフリカ
  'ナイジェリア', 'ガーナ', 'ケニア', 'エチオピア', 'ウガンダ', 'セネガル', 'カメルーン', 'コートジボワール',
  'エジプト', 'モロッコ', 'アルジェリア', 'チュニジア', 'リビア', 'スーダン', 'ソマリア',
  ...
  // 在留資格・入管関連制度キーワード
  '不法滞在', '不法残留', '不法入国', '密入国', 'オーバーステイ', '不法就労', '密航',
  '技能実習生', '元技能実習生', '特定技能', '留学生', '元留学生', '仮放免',
  '偽造在留カード', '在留カード', '入管法', '入管難民法',
  '難民', '難民申請'
];

// 代表的国籍リスト（同一事件の国籍一致・不一致判定用）
const NATIONALITIES = [
  'ベトナム', '中国', '韓国', '台湾', '香港', '北朝鮮', 'モンゴル',
  'フィリピン', 'タイ人', 'タイ国籍', 'タイ籍', 'インドネシア', 'カンボジア', 'ミャンマー', 'ラオス', 'マレーシア', 'シンガポール', 'ブルネイ', '東ティモール',
  'ネパール', 'インド', 'スリランカ', 'パキスタン', 'バングラデシュ', 'ブータン', 'モルディブ', 'アフガニスタン',
  'ウズベキスタン', 'カザフスタン', 'キルギス', 'タジキスタン', 'トルクメニスタン',
  'トルコ', 'クルド', 'イラン', 'イラク', 'シリア', 'レバノン', 'ヨルダン', 'イスラエル', 'パレスチナ',
...
const DOMESTIC_INDICATORS = [
  '日本で', '日本国内', '県警', '警視庁', '府警', '道警', '警察署',
  '在日', '来日', '訪日', '不法就労', '不法残留',
  '地方裁判所', '地裁', '署が', '署に', '署は', '署員',
  '地検', '簡裁', '高裁',
...
const EXCLUDE_KEYWORDS = [
  ...
  'モスク', '追い出して', '追い出し', 'イスラム教礼拝所', '礼拝所', '嫌悪感を抱いた男', '懲らしめてやろう',
  ...
  'CORTIS', 'ＣＯＲＴＩＳ', 'BIGHIT', // 韓国芸能人・K-POP現地事案の完全排除
  '外国人風', '逮捕してよ', '逮捕しろ', // 未検挙・SNS炎上記事の排除
  
  // デマ・不確定情報・事実否定報道・ファクトチェック・偽情報記事の完全排除
  '不確定情報', '事実を把握していない', 'デマ', '事実無根', '裏付ける事実はない', '確認されていない',
  '偽情報', 'ファクトチェック', '誤認させる', '過去動画を流用', '流用し', 'デマ情報', '虚偽の投稿', '虚偽投稿', '偽動画', 'フェイク',
  
  // オピニオン・コラム・発言記事の排除
  '私見', '持論', '語る', '苦言', '言及', '内幕', '捜査の内幕', '氏、', 'ひろゆき',
  
  // 被害者側の刑事告訴・被害申告事案、および論考記事の除外
  ...
  'JPドラゴン', 'ＪＰドラゴン', '日本人女', '日本人男', '日本人男女', '日本人ら', '日本人グループ',
  '移送目的', '移送しようと', 'に潜伏', 'へ潜伏', 'に逃亡', 'へ逃亡', '海外逃亡', '当局が拘束', '日本に移送', '拠点特殊詐欺', 'に派遣',
  
  // ※ 日本人雇用主の排除は、外国籍社長本人の誤遮断を防ぐため isJapaneseSuspect 側で高精度に個別判定
...
// 芸能・エンタメ・週刊誌・ゴシップ・回顧ルポメディアの完全除外リスト
const ENTERTAINMENT_MEDIA = [
  ...
  'NEWSポストセブン', 'ポストセブン', '週刊女性PRIME', '週刊女性', 'Smart FLASH', 'FLASH',
  'FRIDAY', 'フライデー', '文春オンライン', '週刊文春', 'デイリー新潮', '週刊新潮',
  '日刊ゲンダイ', '夕刊フジ', '東スポWEB', '東スポ', 'サイゾー', '日刊サイゾー',
  'Business Journal', '女子SPA!', 'SPA!'
];
...
function detectLocation(title) {
  ...
  title = title.replace(/[（\(〈<【\[〔][^）\)〉>】\]〕]*(?:新聞|テレビ|放送|NEWS|通信|オンライン|デジタル|時事|共同)[^）\)〉>】\]〕]*[）\)〉>】\]〕]$/i, '');
...
function isSameEvent(itemA, itemB) {
  ...
  const locA = (typeof itemA === 'object' && itemA.location && itemA.location !== '全国') ? itemA.location : detectLocation(titleA);
  const locB = (typeof itemB === 'object' && itemB.location && itemB.location !== '全国') ? itemB.location : detectLocation(titleB);
  ...
  // 地域一致または地域不問のエンティティ自動統合（重要単語マッチング）
  if (affA !== 'UNKNOWN' && affA === affB &&
  ...
  if (isOkayamaCaseA && isOkayamaCaseB) {
    return true;
  }

  // 特例：羽田空港・米国籍女2人大麻密輸事件（日テレ/産経/TBS等の統合）
  const isHanedaA = (titleA.includes('大麻') || titleA.includes('乾燥大麻')) && (titleA.includes('米国') || titleA.includes('アメリカ'));
  const isHanedaB = (titleB.includes('大麻') || titleB.includes('乾燥大麻')) && (titleB.includes('米国') || titleB.includes('アメリカ'));
  if (isHanedaA && isHanedaB) {
    return true;
  }

  // 特例：宮城・スリランカ国籍男ら4人空き家窃盗事件（FNN/Excite等の統合）
  const isSrilankaA = (titleA.includes('スリランカ') || titleA.includes('スリランカ人') || titleA.includes('スリランカ国籍')) && (titleA.includes('空き家') || titleA.includes('空き巣'));
  const isSrilankaB = (titleB.includes('スリランカ') || titleB.includes('スリランカ人') || titleB.includes('スリランカ国籍')) && (titleB.includes('空き家') || titleB.includes('空き巣'));
  if (isSrilankaA && isSrilankaB) {
    return true;
  }

  // 特例：中国向け地下銀行・無許可為替取引事件（京都府警等の報道統合）
  const isBankCaseA = (titleA.includes('地下銀行') || titleA.includes('為替取引') || titleA.includes('人民元')) && (titleA.includes('中国') || titleA.includes('京都'));
  const isBankCaseB = (titleB.includes('地下銀行') || titleB.includes('為替取引') || titleB.includes('人民元')) && (titleB.includes('中国') || titleB.includes('京都'));
  if (isBankCaseA && isBankCaseB) {
    return true;
  }

  // 特例：大学生スマホ不正契約・不正送金事件（ベトナム人ら男女4人・700台・警視庁の報道を1本化）
  const isPhoneCaseA = (titleA.includes('スマホ') || titleA.includes('スマートフォン')) &&
                       (titleA.includes('ベトナム') || affA === 'VIETNAM') &&
                       (titleA.includes('不正') || titleA.includes('契約') || titleA.includes('送金') || titleA.includes('大学生') || titleA.includes('4人'));
  const isPhoneCaseB = (titleB.includes('スマホ') || titleB.includes('スマートフォン')) &&
                       (titleB.includes('ベトナム') || affB === 'VIETNAM') &&
                       (titleB.includes('不正') || titleB.includes('契約') || titleB.includes('送金') || titleB.includes('大学生') || titleB.includes('4人'));
  if (isPhoneCaseA && isPhoneCaseB) {
    return true;
  }
  ...
  for (const itemXml of itemMatches) {
    ...
    if (titleMatch && linkMatch) {
      ...
      const isForeignVictim = /(インドネシア|ベトナム|中国|韓国|フィリピン|タイ|ブラジル|ミャンマー|台湾|外国)(人|国籍|籍)?[の男女代性0-9０-９歳（）\s]*[をはがの]?(頭蓋骨|骨折|意識不明|重傷|軽傷|死亡|重体|刺され|被害|だまし取られ|下着|暴行|はね|撥ね|轢き|ひき逃げ)/.test(title) ||
                              /(外国人|外国籍|実習生|留学生)[にへの]?[の男女代性0-9０-９歳（）\s]*(日常的暴行|暴行|傷害|性的暴行|差別|対する|賃金を支払わ|下着|私物|部屋に侵入|はねられ|轢かれ)/.test(title) ||
