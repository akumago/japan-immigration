...
const S = {
  ...
  barRow: {
    display: "grid",
    gridTemplateColumns: "155px 1fr 60px",
    alignItems: "center",
    gap: ".65rem",
    marginBottom: ".45rem",
  } as React.CSSProperties,
  ...
  table: { width: "100%", borderCollapse: "collapse" as const, fontSize: ".85rem" },
  th: (left?: boolean) =>
    ({
      fontSize: ".7rem",
      letterSpacing: 2,
...
export default function BorderlessWelfareState() {
  ...
  return (
    <div ref={ref} style={S.wrap}>
      ...
      {/* ── ヘッダー ── */}
      <div style={S.sec}>
        <div style={{ fontSize: ".72rem", letterSpacing: 3, textTransform: "uppercase", color: "rgba(255,255,255,.35)", marginBottom: ".65rem" }}>
          Amsterdam University School of Economics — Borderless Welfare State (2023)
        </div>
        <div style={{ fontSize: "clamp(1.4rem,3.5vw,2.1rem)", fontWeight: 900, lineHeight: 1.1, color: "#fff", marginBottom: ".5rem" }}>
          移民の出身国別 <span style={{ color: "#c8102e" }}>生涯財政純貢献度</span>の真実
        </div>
        <div style={{ fontSize: ".94rem", color: "rgba(255,255,255,.5)", lineHeight: 1.75 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: "rgba(255,255,255,.06)", borderBottom: "1px solid rgba(255,255,255,.07)" }}>
        {[
          ...
          <div key={k.n} style={{ background: "#09090f", padding: "1rem .75rem", textAlign: "center" }}>
            <span style={{ fontSize: "clamp(1.1rem,2.8vw,1.6rem)", fontWeight: 900, display: "block", lineHeight: 1, color: k.c }}>{k.n}</span>
            <span style={{ fontSize: ".75rem", color: "rgba(255,255,255,.35)", marginTop: ".4rem", display: "block", lineHeight: 1.4, whiteSpace: "pre-line" }}>{k.l}</span>
          </div>
        ))}
      </div>
      ...
      <div style={S.sec}>
        ...
        {PLUS_BARS.map((b) => (
          <div key={b.name} style={S.barRow}>
            ...
            <div style={{ fontSize: ".85rem", fontWeight: 700, textAlign: "right", color: b.color }}>{b.val > 0 ? `+${b.val}` : b.val}</div>
          </div>
        ))}
        <hr style={S.divider} />
        ...
        {MINUS_BARS.map((b) => (
          <div key={b.name} style={S.barRow}>
            ...
            <div style={{ fontSize: ".85rem", fontWeight: 700, textAlign: "right", color: b.color }}>{b.val}</div>
          </div>
        ))}
            ...
            <tbody>
              {TABLE_ROWS.map((r, i) => {
                if ((r as any).header) return (
                  <tr key={i}>
                    <td colSpan={4} style={{ padding: ".35rem .55rem", fontSize: ".75rem", color: "rgba(255,255,255,.25)", letterSpacing: 2, textTransform: "uppercase" }}>{r.r}</td>
                  </tr>
                );
                return (
                  <tr key={i}>
      ...
      <div style={S.sec}>
        ...
        <div style={S.gen2Grid}>
          <div style={S.gen2Card}>
            <div style={{ fontSize: ".7rem", color: "#2ecc71", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: ".75rem" }}>第2世代も改善維持</div>
            {GEN2_PLUS.map((r) => (
              <div key={r.c} style={S.gen2Row}>
                <span style={{ color: "rgba(255,255,255,.75)" }}>{r.c}</span>
                <div style={{ display: "flex", gap: ".65rem", fontWeight: 700, fontSize: ".9rem" }}>
                  <span><span style={{ color: "#555", fontSize: ".7rem" }}>1世: </span><span style={{ color: r.g1col }}>{r.g1}</span></span>
                  <span><span style={{ color: "#555", fontSize: ".7rem" }}>2世: </span><span style={{ color: r.g2col }}>{r.g2}</span></span>
                </div>
              </div>
            ))}
          </div>
          <div style={S.gen2Card}>
            <div style={{ fontSize: ".7rem", color: "#e74c3c", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: ".75rem" }}>第2世代で悪化・固定化</div>
            {GEN2_MINUS.map((r) => (
              <div key={r.c} style={{ ...S.gen2Row, borderBottom: "1px solid rgba(255,255,255,.05)" }}>
                <span style={{ color: "rgba(255,255,255,.75)" }}>{r.c}</span>
                <div style={{ display: "flex", gap: ".65rem", fontWeight: 700, fontSize: ".9rem" }}>
                  <span><span style={{ color: "#555", fontSize: ".7rem" }}>1世: </span><span style={{ color: r.g1col }}>{r.g1}</span></span>
                  <span><span style={{ color: "#555", fontSize: ".7rem" }}>2世: </span><span style={{ color: r.g2col }}>{r.g2}</span></span>
      ...
      <div style={S.sec}>
        ...
        <div style={{ background: "rgba(200,16,46,.06)", border: "1px solid rgba(200,16,46,.14)", borderRadius: 6, padding: "1.1rem 1.25rem", textAlign: "center", marginTop: ".9rem" }}>
          <div style={{ fontSize: ".7rem", letterSpacing: 3, textTransform: "uppercase", color: "#c8102e", marginBottom: ".55rem" }}>報告書の最終結論</div>
          <div style={{ fontWeight: 900, fontSize: "clamp(.95rem,2.5vw,1.35rem)", color: "#fff", letterSpacing: .5, lineHeight: 1.4 }}>
            "YOU CANNOT SIMULTANEOUSLY HAVE FREE IMMIGRATION AND A WELFARE STATE"
          </div>
          <div style={{ fontSize: ".7rem", color: "rgba(255,255,255,.35)", marginTop: ".5rem" }}>
      ...
      <div style={S.sec}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          {JAPAN_RISKS.map((risk) => (
            <div key={risk.title} style={{ background: "rgba(255,107,53,.04)", border: "1px solid rgba(255,107,53,.15)", borderRadius: 8, padding: "1rem 1.1rem" }}>
              <div style={{ fontSize: "1rem", marginBottom: ".65rem", display: "flex", alignItems: "center", gap: ".5rem" }}>
                <span>{risk.icon}</span>
                <span style={{ fontWeight: 700, color: "#fff", fontSize: ".9rem" }}>{risk.title}</span>
              </div>
              <ul style={{ paddingLeft: "1.1rem", margin: 0 }}>
                {risk.items.map((item, i) => (
                  <li key={i} style={{ fontSize: ".85rem", color: "rgba(255,255,255,.6)", lineHeight: 1.7, marginBottom: ".25rem" }}>{item}</li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 6, padding: "1rem 1.1rem" }}>
          <div style={{ display: "grid", gap: ".65rem" }}>
            {[
              ...
              <div key={n.tag} style={{ display: "flex", gap: ".75rem", alignItems: "flex-start" }}>
                <div style={{ flexShrink: 0, marginTop: ".3rem" }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: n.col }} />
                </div>
                <div>
                  <span style={{ fontSize: ".75rem", fontWeight: 700, color: n.col, letterSpacing: .5 }}>{n.tag}　</span>
                  <span style={{ fontSize: ".85rem", color: "rgba(255,255,255,.5)", lineHeight: 1.75 }}>{n.text}</span>
                </div>
              </div>
            ))}
          </div>
